...
const filteredEmojis = emoji.filter(
  e => (isAndroid ? !!e.google : !includes(blacklistedEmojis, e.short_name))
);
...
class EmojiPicker extends Component {
  ...
  loadNextCategory() {
    if (this.state.categories.length < CATEGORIES.length) {
      this.setState({
        categories: CATEGORIES.slice(0, this.state.categories.length + 1)
      });
    }
  }
...
class EmojiCategory extends Component {
  ...
  render() {
    ...
    let style = {
      ...
      color: "black",
      //height: size + 8,
      height: isAndroid ? size + 16 : size + 10,
      width: isAndroid ? size + 14 : size + 6,
      textAlign: "center",
      padding: padding
    };
    ...
    return (
      <View style={style.categoryOuter}>
        <Text style={[styles.headerText, this.props.headerStyle]}>
          {this.props.category}
        </Text>
...
const EmojiOverlay = props => (
  <View
    style={[styles.absolute, props.visible ? styles.visible : styles.hidden]}
  >
